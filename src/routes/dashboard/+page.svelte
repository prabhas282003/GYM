<script>
	...
	// Fetch overdue tasks
	$: overdueTasks = $taskStore.filter(task => {
  ...
  const rescheduleTasks = () => {
    if (selectedTask && selectedDateTime) {
	taskStore.manuallyRescheduleTask(selectedTask.id, new Date(selectedDateTime));
	showModal = false;
        ...
        >
			...
			<div class="modal">
				<div class="modal-content">
					<h2>Select a Task to Reschedule</h2>
					<ul>
					...
					<button on:click={() => showTaskSelection = false} class="p-2 bg-gray-500 text-white rounded mt-4">Cancel</button>
				</div>
			</div>
			{/if}
